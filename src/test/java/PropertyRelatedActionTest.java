...
class PropertyRelatedActionTest {
    ...
    void buylandTest() {
        ...
        y1.updatePosition(p1,l1.position);
        y1.buyland(p1);
        y1.updateProperty(p1);
        Assertions.assertEquals(p1.getId(),l1.getOwner());
        Assertions.assertEquals(pro,p1.getPropertyList());
    }
    ...
    void updatePropertyTest() {
        ...
        y2.updatePosition(p2,l2.position);
        y2.buyland(p2);
        y2.updateProperty(p2);
        Assertions.assertEquals(p2.getId(),l2.getOwner());
        Assertions.assertEquals(pro2,p2.getPropertyList());
    }
}
